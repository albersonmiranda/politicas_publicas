...
#| results: asis
#| label: tab-q4.d
#| tbl-cap: Coeficientes da forma reduzida
...
#| results: asis
#| label: tab-q4.e
#| tbl-cap: Coeficientes do primeiro estágio
...
#| results: asis
#| label: tab-q4.f
#| tbl-cap: Coeficientes com nearc4 como instrumento
...
#| results: asis
#| label: tab-q4.h
#| tbl-cap: Regressão de IQ contra nearc4
...
#| results: asis
#| label: tab-q4.i
#| tbl-cap: Regressão de IQ contra nearc4 com variáveis regionais
...
Essa questão replica a análise contida no artigo “Evaluating the Effect of Homicide Prevention Strategies in São Paulo, Brazil: A Synthetic Control Approach”de Danilo Freire (2010). O autor busca mensurar o efeito causal agregado de políticas públicas sobre as taxas de homicídios em São Paulo. O artigo está disponível no link; familiarize-se com ele antes de começar. Você precisará usar o arquivo df.Rdata para realizar o exercício a seguir. Descrição da base: O painel de dados contém informações de 1990 a 2009, para os 26 estados e o Distrito Federal. Os seguintes estados constam na base, mas não são utilizados na análise: Bahia e Sergipe.

- Formato: dataframe - painel.
- 1 unidade tratada: nome do estado: São Paulo.
- 24 regiões de controle.
- 20 anos (1990:2009).
- Identificadores:
  ...
  - year: ano
- Variável dependente:homicide.rates (número de homicídios por 100.000 habitantes)
- Preditores: 1990:1998:
  - homicide.rates: taxa de homicídios (número de homicídios por 100.000 habitantes);
  - state.gdp.capita: PIB per capita estadual (em reais de 2010);
  - state.gdp.growth.percent: crescimento do PIB estadual (percentual, em reais de 2010);
  - population.projection.ln: logaritmo natural da população (projeções anuais da população do estado);
  - proportion.extreme.poverty: população vivendo em extrema pobreza (percentual da população do estado que nãoatinge a ingestão mínima de 2.000 calorias por dia);
  - years.schooling.imp: anos de escolaridade (número  médio  de  anos  de  instrução formal em estabelecimentos de ensino-homens e mulheres com 25 anos ou mais). As observações ausentes foram imputadas com interpolação linear;
  - gini.imp: índice de Gini (medida de desigualdade que varia de 0 a 1, onde 0 é o mais igual e 1 o mais desigual). As observações ausentes foram imputadas com interpolação linear.
- Para organizar os dados, use os seguintes parâmetros na função dataprep():
  - predictors.op = "mean";
...
Discuta brevemente a estratégia de identificação empregada pelo autor ressaltando suas vantagens em relação a estudos de caso comparativos qualitativos. Indique também uma crítica a abordagem seguida pelo autor.

## b

Obtenha os pesos para as covariadas (preditores) e para os estados doadores para construção do São Paulo sintético. Organize os resultados como na tabela 1 do artigo.

## c

Elabore uma tabela que compara os valores dos preditores pré-tratamento para a unidade tratada, a unidade sintética de controle e todas as unidades na amostra (Tabela 2, no artigo). Comente sobre a semelhança entre SP real e seu controle em cada uma das dimensões.

## d

Elabore dois gráficos ilustrativos dos resultados: um mostrando a evolução da trajetória da taxa de homicídios em SP e em seu controle (figura 2 do artigo) e outro mostrando o gap entre a taxa de homicídios em SP e seu controle (figura 3 do artigo) e comente brevemente os resultados.

## e

Escolha o estado não tratado com maior peso no controle sintético e faça um teste de placebo tentando encontrar algum  efeito da política sobre esse estado.  Comente seu resultado ressaltando sua implicação para a análise.

## f

Aumente essa análise para todos os demais estados e coloque os resultados em um gráfico ilustrativo (figura 6 do artigo) fazendo o teste de permutação sugerido por Abadie Diamond e Hainmueller (2010). Comente brevemente os resultados.